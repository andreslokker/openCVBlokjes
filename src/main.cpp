...
int apertureSize = 3;
double k = 0.04;
void applyFilters(const Mat& img, Mat imgHsv, Mat& imgTreshold, Mat& imgMultiply, Mat& imgEdges, Mat& drawing)
{
	...
	dilate(imgTreshold, imgTreshold, getStructuringElement(MORPH_RECT, Size(9,9), Point(-1,-1)));
	  ...
	  cvtColor(imgTreshold, imgTreshold, COLOR_GRAY2BGR);
	    ...
	    Canny(imgTreshold, imgTreshold, 50, 100);
	    ...
	    drawing = Mat::zeros( imgTreshold.size(), CV_8UC3 );
	    RNG rng(12345);
...
//    namedWindow("img detected");
    ...
    while(true) {
    	...
    	applyFilters(frame, imgHsv, imgTreshold, imgMultiply,  imgEdges, drawing);
    	imshow("live", frame);
